...
def histogram(img, filename):
    ...
    x = list(range(256))
    axarr[0].bar(x, height=r_hist, color='red', alpha=.64, width=1)
    axarr[0].set_title('Histograma Vermelho', fontsize=8)
    axarr[1].bar(x, height=g_hist, color='green', alpha=.64, width=1)
    axarr[1].set_title('Histograma Verde', fontsize=8)
    axarr[2].bar(x, height=b_hist, color='blue', alpha=.64, width=1)
